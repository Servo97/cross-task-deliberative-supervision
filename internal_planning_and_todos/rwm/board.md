...
| RoboMME preflight | fires #5/#6 reach the 8 canary lanes; one lane exits 1 (gpu3, then gpu4) and aborts the rest; lane logs were never shipped (#6's handler synced the wrong dir). **Fire #7 `p5-native-eval-v1-0fdd740cb94e03ccd713` at 400** with diagnostics shipped to `<id>/failure/<utc>/evidence/` on failure (repo `95e04c7`, §56.5–56.6). On claim → `ready/fire_rmme_campaign.sh` | — |
