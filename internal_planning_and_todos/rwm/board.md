...
| RoboMME preflight | fire #2 `fa05c929…` (400) FAILED 20:21Z: identity check PASSED, then `ModuleNotFoundError: launch_guardrails` (bundle ships only `robomme_integration/`; `launch.py` imports from `scripts/launch`). Fix: eval launchers vendor `launch_guardrails.py` + `wsm_settings.py` into `robomme_integration/_vendor/` (opt-in `vendor_files`), `launch.py` falls back to it; node-import test added; 27 tests pass; repo `4a21129`. **Fire #3 `p5-native-eval-v1-9747ba48a4f744b3c2fe` QUEUED 20:28Z at 400**, arn `…/69ba7b2e-9858-4696-b4cb-3051d8ac561f`, source tree `0dd0370c…`. On claim → `ready/fire_rmme_campaign.sh` (112 cells, 400) | — |
