...
| RoboMME preflight | fire #8 `0b3ea564…`: servers UP (trainer admission works); shards died — the registered v0.4.0 runtime ships a **CPU-only torch** (built for the lavapipe/CPU-render path) while native-EGL lanes read frames back through CUDA torch. Entries now overlay torch 2.9.1+cu128 into the simulator site (repo `61dc9db`, §56.8). **Fire #9 `p5-native-eval-v1-93eb1f24198da37de5e0` at 400**. On claim → `ready/fire_rmme_campaign.sh` | — |
