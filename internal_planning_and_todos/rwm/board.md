...
| RoboMME preflight | fires #2 (bundle import), #3 (node-side priority pin), #4 (canary template binding name `scientific_spec_sha256_v1`, never a known mode) FAILED — each one node-only check further; all three now caught by local tests (repo `fadcd4b`). **Fire #5 `p5-native-eval-v1-6e3b28e40cf84fc0e4b3` QUEUED 20:58Z at 400**, arn `…/4b47d3a0-8e54-4b9f-af66-64fe7881faab`, source tree `5b4a076c…` (§56.2–56.4). On claim → `ready/fire_rmme_campaign.sh` (112 cells, 400) | — |
