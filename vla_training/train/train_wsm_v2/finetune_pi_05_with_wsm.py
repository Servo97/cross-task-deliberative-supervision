#!/usr/bin/env python3
"""WSM-v2 finetune of pi0.5: action loss + 3D-flow (and SigReg) aux heads (jointly trained).
...
modules/networks live in workspace_models. Runs on robocasa_openpi @ jax-latest. Governed by
internal_planning_and_todos/04_wsm_roadmap.md ("R3 = E2 — 3D flow (WSM v2)" + "E3 — SigReg / LeJEPA").

TODO
----
- [ ] Load the YAML recipe (default: scripts/configs/train/wsm_v2/pi05_wsm_v2.yaml; configs TBD).
- [ ] Start from the WSM-base (or R0) pi0.5 orbax checkpoint as the base VLA.
...
@dataclass
class WsmV2Config:
    """Typed view of the YAML recipe (fields TODO — base pi0.5 recipe + flow/sigreg head knobs)."""

    base_checkpoint: str = "ckpts/wsm_base/pi_05"
    flow_loss_weight: float = 1.0
    sigreg_loss_weight: float = 0.0
    flow_labels_dir: str = "data/wsm_labels/flow3d"
    output_dir: str = "ckpts/wsm_v2/pi_05"
...
def main() -> None:
    ...
    print(f"[finetune_pi_05_with_wsm/v2] resolved config: {cfg}")
    raise NotImplementedError("TODO: attach 3D-flow (+sigreg) WSM heads to pi0.5 and train jointly")
